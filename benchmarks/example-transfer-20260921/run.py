...
def add_example(package):
    destination = package / 'examples/document-publisher'
    destination.mkdir(parents=True, exist_ok=False)
    shutil.copy2(HERE / 'example/publisher.architecture.json', destination / 'publisher.architecture.json')
    shutil.copytree(HERE / 'example/source', destination / 'source')
    shutil.copy2(HERE / 'example/source-pin.json', destination / 'source-pin.json')
    shutil.copy2(HERE / 'example/preview.png', destination / 'preview.png')
    shutil.copy2(HERE / 'reference.md', package / 'references/library-worked-example.md')
    skill = package / 'SKILL.md'
    old = skill.read_text()
    anchor = '3. Artifact first:'
    assert old.count(anchor) == 1
    skill.write_text(old.replace(anchor, '   ' + (HERE / 'entrypoint.txt').read_text().strip() + '\n\n' + anchor))


def candidate_package_files(package):
    import tempfile
    with tempfile.TemporaryDirectory() as directory:
        copy = pathlib.Path(directory) / 'archify'
...
def freeze(args):
    ...
    result = {
        ...
        'intervention': {p.name: sha(p) for p in [HERE / 'reference.md', HERE / 'entrypoint.txt', HERE / 'tasks.json', HERE / 'protocol.json', HERE / 'review-key.json',
        ...
        'screening': read(HERE / 'protocol.json')['screen'],
        'public_trace_extractor_sha256': sha(PREVIOUS / 'trace_extract.py'),
        ...
        'timing': 'T_full begins before per-run copying/dispatch and ends at native task_complete; common remote clone/research/review costs reported separately. Example reading is included; first structural JSON is diagnostic only.',
    ...
    result['initial_order'] = result['protocol']['authors']['initial_order']
    result['example'] = tree(HERE / 'example')
    result['candidate_package'] = candidate_package_files(WORKTREE / 'archify')
    result['experiment'] = result['protocol']['conditions']
...
def begin(args):
    ...
    assert sha(PREVIOUS / 'trace_extract.py') == frozen['public_trace_extractor_sha256'], 'Trace extractor changed'
    ...
    if condition.startswith('P'):
        assert tree(HERE / 'example') == frozen['example'], 'Frozen example changed'
        add_example(workspace / 'archify')
...
Deliver an English source-backed Architecture showcase in output/diagram.architecture.json and output/diagram.html. Keep exact repository metadata and source evidence. Use the complete normal finalize command with --repo-root ./repo --quality showcase --json. All source/example reading, authoring, repairs and final response count toward time. Allow at most three focused repairs after the first finalize; if still blocked, report the failure without changing meaning or quality. Stop within 600 seconds. Do not claim machine success is semantic/visual human acceptance.
...
'''
    common += '\nFollow the local packaged Skill and its referenced examples.\n'
    ...
    observer = subprocess.Popen([sys.executable, str(HERE / 'watch.py'),
                                ...
                                '--artifact', 'diagram.architecture.json', '--cap', '900'],
    ...
    write(result / 'start.json', {'full_start_epoch': started, 'workspace': str(workspace),
                                ...
                                'condition': condition, 'task_id': task_id, 'instructions_sha256': sha(workspace / 'TASK.md')})
...
def finish(args):
    ...
    def categories(command):
        ...
        if 'library-worked-example.md' in command:
            values.append('worked-example-reference-access')
        if 'publisher.architecture.json' in command or 'document-publisher' in command:
            values.append('worked-example-file-access')
    ...
    frozen = read(args.out / 'freeze.json')
    expected_package = frozen['candidate_package'] if start['condition'].startswith('P') else frozen['package']
    integrity = {'instructions_unchanged': sha(workspace / 'TASK.md') == start['instructions_sha256'],
                 'source_unchanged': tree(workspace / 'repo') == frozen['sources'][start['task_id']]['files'],
