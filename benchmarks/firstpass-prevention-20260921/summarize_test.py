...
spec = importlib.util.spec_from_file_location('prevention_summary', pathlib.Path(__file__).with_name('summarize.py'))
    ...
    def test_initializing_receipt_is_not_failure_and_pre_finalize_change_is_visible(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            write(out / 'freeze.json', {'initial_order':['fixture-B1','fixture-P1'],
                'tasks':{'tasks':[{'id':'fixture'}]}, 'protocol':{'success_screen':{
                'minimum_both_quality_pairs':2, 'median_full_time_seconds_saved':20,
                'median_full_time_fraction_saved':.15, 'median_model_repair_reduction':1,
                'maximum_qualified_pair_slowdown_fraction':.1}}})
            run = out / 'fixture-P1'
