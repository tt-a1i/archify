...
No install is required inside the skill package. The skill root is the directory that contains this `SKILL.md`. Invoke the CLI as `node <skill-root>/bin/archify.mjs …` from any working directory; do not assume the shell cwd is the skill root. Verify with:
