...
Hosted CI completed successfully on `dc2153b` (run `34679315784`), including the existing stored-motion-preference assertions with additional browser storage diagnostics. This does not establish the root cause of the two earlier intermittent failures. Existing #373 also completed all hosted gates on `e59c633` (run `34679027097`); it remains unmerged.

The final handoff audit found and closed specific omissions: field-level map diagnostics with repair suggestions, real receipt tests replacing a source-text assertion, lint semantics and bundle error/API documentation, three README entry links, and six fixed-revision source citations. Ordinary Locate's optional child sidecar was distinguished from required parent and bundle inputs instead of changing its established contract. Forty-four focused contract checks passed.

Bundle listener registration now waits for document parsing and requires the relevant manifest/child context. An initial immediate-manifest guard failed real navigation because bundle injection can place its JSON after the runtime; the parsing-aware registration passed all six browser cases. Runtime observation confirms an ordinary Viewer registers no bundle message listener, while a genuine child without its own manifest/projection still acknowledges and returns. This regression found and rejected the incomplete guard before publication.

All affected HTML, Gallery, Checkout comparison, pinned MCO artifacts, README animation and the official Node 22 ZIP were regenerated. The manually authored web-app SVG bytes remain identical. The current PR head's checks and description are the final integration receipt; historical failures above remain historical. The [disposition record](identity-map-legacy-disposition-2026-09-12.md) maps each original requirement to implementation or an explicit decision and records why old presets, extra uncovered widgets and a simulated human study were not adopted.


## Stored-motion test correction

The full local suite on immutable `0e38aa9` completed with **1,489 total, 1,437 passed, zero failed and 52 skipped**, with tracked source hashes unchanged. Its hosted run `34683543153` passed Node/package/ZIP/manifest gates but failed the stored-motion assertion again. Instrumentation captured a write of `still` before navigation and an empty value at the next document's earliest startup; no intervening Viewer deletion was captured. This does not prove a particular Chromium internal fault.

The test had changed the query of a `file:` URL on every iteration while describing those navigations as reloads. The [URL Standard leaves file-origin behavior implementation-defined](https://url.spec.whatwg.org/#origin), while [Web Storage associates local storage with the document's origin](https://html.spec.whatwg.org/multipage/webstorage.html#the-localstorage-attribute). The corrected test exercises five actual `Page.reload` calls on the unchanged file URL, verifying a new loader and navigation type, plus five cross-URL navigations between real generated diagrams on one local HTTP origin. Both cases assert the stored startup value, current value and Governor mode; reduced motion, suspension, non-persisting changes and unavailable-storage coverage remain intact. Temporary prototype instrumentation was removed after retaining its failure evidence. No product behavior or package content changes in this correction.

The corrected file passed **9/9 tests, zero skips** with Chrome 153.0.8010.36 and Node 22.23.2. This test-only correction uses that focused local run and the current head's hosted CI; the full local result above remains explicitly bound to its product commit.


### Linux control identifies an interfering diagnostic read

The corrected-navigation candidate `d83f5ce` still failed its second actual file reload in [run 34684258289](https://github.com/tt-a1i/archify/actions/runs/34684258289), while all five same-origin HTTP navigations passed. This disproved query-changing navigation as a sufficient explanation.

Two bounded diagnostic runs used exact Linux Chrome 153.0.8010.36, fresh profiles and twenty unchanged-URL reloads per condition. [Run 34684758824](https://github.com/tt-a1i/archify/actions/runs/34684758824) compared tiny plain HTML and generated Viewer with/without an injected early `localStorage.getItem`, and with/without six prior reset/navigate cycles. Both Viewer-plus-early-read cases lost data; the other six cases retained it. [Run 34684880899](https://github.com/tt-a1i/archify/actions/runs/34684880899) replaced tiny HTML with script-free HTML padded to the Viewer's size: both padded controls with early reads also lost data, while both padded controls and both Viewers without early reads retained all twenty reloads. The full per-navigation observations and browser storage events are attached to those runs. The diagnostic job records observations; its successful exit does not mean the loss assertions passed.

This isolates a test-injected pre-initialization read as an interfering condition and establishes that Viewer code is not required to reproduce the failure. It does not establish a particular Chromium internal mechanism or promise all file-origin behavior. The browser regression removes that diagnostic read and its corresponding observation-only assertions; the existing persistence contract remains checked through actual post-load localStorage values and Governor mode on every reload/navigation. Reduced motion, suspension, explicit/non-persisting intent and unavailable-storage assertions remain. No Viewer implementation, storage fallback, fixed delay or package content is changed.
