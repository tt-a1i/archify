...
`meta.locale=en|zh-CN|ko` localizes page title, Legend, states/errors, a11y, HTML/SVG `lang`—never authored content. Otherwise omit; preserve requested-language copy; disclose English fallback. Static omits `animation`; `classic` defaults.
...
| **GitHub Actions** | `uses: casareborgia/archify@<commit>` in a workflow ([action.yml](action.yml)) | Deliver diagrams on push, comment Architecture Delta summaries on pull requests, or draft a diagram from code. [Details](integrations/github-action/README.md). |
