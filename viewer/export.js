...
/* ARCHIFY:EXPORT_CLEANUP */
      ...
      function serializeSvg(scale, opts) {
        ...
        if (opts.figure && (document.documentElement.getAttribute('data-preset') || 'classic') !== 'blueprint') {
          // Framed figures sit on a margin; a grid that stops at the canvas edge
          // reads as a stray box. Blueprint keeps it because the grid is its style.
          style.textContent += "\nrect[fill=\"url(#grid)\"] { display: none; }\n";
        }
      ...
      // Raster exports read as a finished figure: a margin around the canvas
      // and the diagram title (plus subtitle) above it, set in the Viewer's
      // own heading face. Values are CSS pixels before the raster scale.
      var FRAME_PADDING = 44;
      var FRAME_TITLE_SIZE = 24;
      var FRAME_SUBTITLE_SIZE = 13;
      ...
      function rasterFrame() {
        ...
        var titleStyle = titleNode ? getComputedStyle(titleNode) : null;
        var computed = getComputedStyle(document.documentElement);
        var header = title ? FRAME_TITLE_SIZE + (subtitle ? 10 + FRAME_SUBTITLE_SIZE : 0) + 12 : 0;
        return {
          title: title,
          subtitle: subtitle,
          family: (titleStyle && titleStyle.fontFamily) || 'sans-serif',
          text: computed.getPropertyValue('--text').trim() || '#0f172a',
      ...
      function rasterize(format) {
        ...
        var svg = document.querySelector('.diagram-container svg');
        var vb = svg.viewBox.baseVal;
        var frame = rasterFrame();
        var frameWidth = vb.width + FRAME_PADDING * 2;
        var frameHeight = vb.height + FRAME_PADDING * 2 + frame.header;
        var scale = pickSafeScale(frameWidth, frameHeight);
        ...
        return new Promise(function (resolve, reject) {
          ...
          img.onload = function () {
            try {
              ...
              ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--bg').trim() || currentBg();
              ctx.fillRect(0, 0, canvas.width, canvas.height);
              if (frame.title) {
                var textWidth = (frameWidth - FRAME_PADDING * 2) * scale;
                ctx.textBaseline = 'alphabetic';
                ctx.fillStyle = frame.text;
                var titleBaseline = (FRAME_PADDING + FRAME_TITLE_SIZE * 0.82) * scale;
                ctx.fillText(fitCanvasText(ctx, frame.title, textWidth, FRAME_TITLE_SIZE * scale, 14 * scale, '650', frame.family),
                  FRAME_PADDING * scale, titleBaseline);
                if (frame.subtitle) {
                  ctx.fillStyle = frame.muted;
                  ctx.fillText(fitCanvasText(ctx, frame.subtitle, textWidth, FRAME_SUBTITLE_SIZE * scale, 10 * scale, '400', frame.family),
                    FRAME_PADDING * scale, titleBaseline + (10 + FRAME_SUBTITLE_SIZE) * scale);
                }
              }
              // Draw at natural size — SVG was already rasterized at target res.
              ctx.drawImage(img, FRAME_PADDING * scale, (FRAME_PADDING + frame.header) * scale);
