    Archify.view = (function () {
      ...
      function zoom(next, options) {
        ...
        var clientX = Number(options.clientX);
        var clientY = Number(options.clientY);
        ...
        if (!(Number.isFinite(anchorX) && Number.isFinite(anchorY)) &&
          ...
          if (rect && rect.width > 0 && rect.height > 0) {
            anchorX = clientX - rect.left + state.x;
            anchorY = clientY - rect.top + state.y;
          }
        }
      ...
      function scrollableWheelTarget(node, horizontal) {
        var current = node;
        while (current) {
          var overflowX = '';
          var overflowY = '';
          ...
          if (horizontal) {
            if (current.scrollWidth > current.clientWidth + 1 &&
                (overflowX === 'auto' || overflowX === 'scroll' || overflowX === 'overlay')) return true;
          } else if (current.scrollHeight > current.clientHeight + 1 &&
      ...
      function zoomByWheel(event) {
        ...
        var deltaX = event.deltaMode === 1 ? (Number(event.deltaX) || 0) * 16 :
          event.deltaMode === 2 ? (Number(event.deltaX) || 0) * (svg.clientWidth || 1) : (Number(event.deltaX) || 0);
        var delta = event.deltaMode === 1 ? event.deltaY * 16 :
          event.deltaMode === 2 ? event.deltaY * (svg.clientHeight || 1) : event.deltaY;
        if (!delta || !Number.isFinite(delta)) return;
        var horizontal = event.shiftKey || Math.abs(deltaX) > Math.abs(delta);
        var overlay = target.closest
          ? target.closest('[data-legend], .diagram-nav, .focus-chip, .node-finder, .diagram-guide, .overview-map, .route-probe, .semantic-lens')
          : null;
        // The axis is classified once, from normalized deltas, and the same
        // result decides both the scrollable ancestor and zoom eligibility.
        // A gesture the camera does not consume keeps native scrolling: only a
        // wheel that will actually move the camera may suppress the default.
        if (scrollableWheelTarget(target, horizontal) || overlay || horizontal) {
          wheelZoomAccum = 0;
          return;
        }
        var zoomingIn = delta < 0;
        if (zoomingIn ? state.scale >= 3 : state.scale <= 1) {
          wheelZoomAccum = 0;
          return;
        }
        event.preventDefault();
        container.classList.add('is-wheel-zooming');
        if (wheelZoomTimer) clearTimeout(wheelZoomTimer);
        wheelZoomTimer = setTimeout(function () {
          wheelZoomTimer = 0;
          wheelZoomAccum = 0;
          container.classList.remove('is-wheel-zooming');
        }, 120);
        var step = 60;
        wheelZoomAccum += delta;
        while (wheelZoomAccum <= -step || wheelZoomAccum >= step) {
          var direction = wheelZoomAccum < 0 ? 1 : -1;
          wheelZoomAccum += direction * step;
          zoom(state.scale + direction * 0.25, {
      ...
      container.addEventListener('wheel', zoomByWheel, { passive: false });
