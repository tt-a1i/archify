    Archify.guidedViews = (function () {
      ...
      // A suppressed effect is one the document declared it does not want. The
      // id belongs to the chrome that owns it; the viewer only asks whether the
      // document listed it, so a new effect never edits a compiler-side list.
      var suppressedEffects = String(svg && svg.getAttribute('data-suppressed') ? svg.getAttribute('data-suppressed') : '').split(/\s+/);
      function effectSuppressed(id) {
        return suppressedEffects.indexOf(id) !== -1;
      }
      var views = [];
      var activeIndex = -1;
      var playing = false;
      ...
      function buildChapterIndex() {
        ...
        views.forEach(function (view, index) {
          ...
          var delta = null;
          if (!effectSuppressed('chapter_delta')) {
          ...
          if (delta) button.appendChild(delta);
      ...
      function syncChapterIndex() {
        chapterButtons.forEach(function (button, index) {
          ...
          if (current) {
            stops.hidden = false;
            if (deltaLabel) deltaLabel.hidden = true;
          ...
          } else if (effectSuppressed('chapter_delta')) {
            // The document asked for no chapter delta: the button keeps the copy
            // it was built with, and nothing derived from the delta reaches the
            // DOM, aria or a tooltip. A suppressed effect owns all of its output.
            stops.hidden = false;
            button.removeAttribute('data-chapter-delta');
            button.setAttribute('aria-label', viewerText('viewer.guided.chapter.open', {
              index: index + 1,
            ...
            button.title = views[index].label + ' \u2014 ' + (views[index].note || viewerText('viewer.guided.chapter.selectedNodes', {
              count: views[index].focus.length
            }));
          } else {
