...
export const PACK_DEFAULT_LIMITS = Object.freeze({
  ...
  maximumOutlineFiles: 16,
  maximumOutlineDefinitions: 12,
});
...
// --- File outlines -----------------------------------------------------------
// The author reads a handful of source files to verify relationships. An
// outline gives each likely file its length and top-level definitions, so the
// author reads the line ranges it needs instead of whole files and never
// cites a line past the end of a file.
const OUTLINE_LANGUAGE = [
  [/\.pyw?$/i, /^(?:async\s+)?def\s+(\w+)|^class\s+(\w+)|^if\s+__name__\s*==\s*['"]__main__['"]/],
  [/\.(?:[cm]?[jt]sx?)$/i, /^(?:export\s+(?:default\s+)?)?(?:async\s+)?function\s*\*?\s*(\w+)|^(?:export\s+(?:default\s+)?)?(?:abstract\s+)?class\s+(\w+)|^(?:export\s+)?(?:const|let)\s+(\w+)\s*=\s*(?:async\s*)?(?:\([^)]*\)|\w+)\s*=>/],
];

function fileOutline(root, file, maximumDefinitions) {
  const language = OUTLINE_LANGUAGE.find(([extension]) => extension.test(file));
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const match = language[1].exec(lines[index]);
    if (!match) continue;
    const name = match.slice(1).find(Boolean) || '__main__';
    if (safeSourceLine(name) !== null) definitions.push(`${index + 1} ${name}`);
  }
  return {
    lines: lineCount,
    defs: definitions.slice(0, maximumDefinitions),
    ...(definitions.length > maximumDefinitions ? { moreDefs: definitions.length - maximumDefinitions } : {}),
  };
}

// Files the author is most likely to read: runtime channel anchors, question
// anchors, then each shown module's entrypoints and sample file.
function outlineCandidates(runtimeChannels, questions, modules) {
  const files = [
    ...runtimeChannels.map((entry) => entry.anchor),
    ...questions.flatMap((question) => question.anchors),
    ...modules.flatMap((module) => [...(module.entrypoints || []), ...(module.sample ? [module.sample] : [])]),
  ].map((anchor) => anchor.replace(/:\d+$/, ''))
    // Hidden folders hold agent, editor and CI tooling, not runtime source.
...
export function buildEvidencePack({ repositoryState, summary, records, boundaries: declaredBoundaries, level2, detailPath, declaredDependencies, root }, options = {}) {
  ...
  const packOutlines = (runtimeChannels, packedQuestions, modules) => {
    if (!rootIdentity || state.outlineFiles === 0) return null;
    const files = {};
    for (const file of outlineCandidates(runtimeChannels.items, packedQuestions, modules.items)) {
      if (Object.keys(files).length >= state.outlineFiles) break;
      if (!outlineCache.has(file)) outlineCache.set(file, fileOutline(rootIdentity, file, limits.maximumOutlineDefinitions));
      const outline = outlineCache.get(file);
      if (!outline) continue;
      const hidden = outline.defs.length - state.outlineDefinitions + (outline.moreDefs || 0);
      files[file] = { lines: outline.lines, defs: outline.defs.slice(0, state.outlineDefinitions), ...(hidden > 0 ? { moreDefs: hidden } : {}) };
    }
    return Object.keys(files).length ? {
      note: 'lines is the file length; defs are "line name" top-level definitions. Read the ranges you need and cite only lines within lines.',
      files,
    } : null;
  };
  const measure = (pack) => {
  ...
  const assemble = () => {
    ...
    outlines: packOutlines(packRuntimeChannels(level2, roles, state.channels, false), questions.slice(0, state.questions), modules),
  ...
  const shrinkSteps = [
    ...
    ['outline-defs-6', () => { state.outlineDefinitions = Math.min(state.outlineDefinitions, 6); }],
    ['outlines-8', () => { state.outlineFiles = Math.min(state.outlineFiles, 8); }],
    ['outlines-4', () => { state.outlineFiles = Math.min(state.outlineFiles, 4); }],
    ...
    ['outlines-0', () => { state.outlineFiles = 0; }],
