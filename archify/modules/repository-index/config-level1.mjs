...
// Returns the top-level string items of a TOML array that starts at `start`
// (the index just after its opening bracket). Brackets inside quoted strings,
// such as PEP 508 extras in "pkg[extra]>=1", do not close the array, and
// comments are skipped, so a lazy regex cannot end the list early.
function tomlStringArray(text, start) {
  const items = [];
  let depth = 1;
  let index = start;
  while (index < text.length && depth > 0) {
    ...
    if (character === '"' || character === "'") {
      ...
      if (close === -1) break;
      if (depth === 1) items.push(text.slice(index + 1, close));
      index = close + 1;
      continue;
    }
...
// A PEP 508 requirement starts with its distribution name; extras, version
// specifiers, and environment markers follow and are not package names.
function requirementName(value) {
  return value.trim().match(/^([A-Za-z0-9][A-Za-z0-9._-]*)/)?.[1] || null;
}

// Console scripts name an importable module (`name = "pkg.cli:main"`); keep the
// module so the evidence pack can map each script onto a scanned file.
function scriptModules(section) {
  return Object.fromEntries(limited([...section.matchAll(/^[ \t]*([A-Za-z0-9_.-]+)\s*=\s*["']?([A-Za-z_][\w.]*)(?::[\w.]+)?["']?[ \t]*$/gm)]
    .map((match) => [match[1], match[2]])));
}

function parsePyproject(text) {
  const dependencyList = /^dependencies\s*=\s*\[/m.exec(text);
  const projectDependencies = dependencyList ? tomlStringArray(text, dependencyList.index + dependencyList[0].length) : [];
  const poetry = first(text, /^\[tool\.poetry\.dependencies\]\s*([\s\S]*?)(?=^\[|(?![\s\S]))/m) || '';
  const scripts = first(text, /^\[project\.scripts\]\s*([\s\S]*?)(?=^\[|(?![\s\S]))/m) || '';
  return {
    name: first(text, /^name\s*=\s*["']([^"']+)["']/m),
    dependencies: unique([
      ...projectDependencies.map(requirementName).filter(Boolean),
      ...matches(poetry, /^([A-Za-z0-9_.-]+)\s*=/gm),
    ]),
    scripts: matches(scripts, /^([^#=\s]+)\s*=/gm),
    scriptModules: scriptModules(scripts),
  };
}
...
function parseSetupCfg(text) {
  ...
  return {
    ...
    scriptModules: scriptModules(first(text, /^\[options\.entry_points\]\s*([\s\S]*?)(?=^\[|(?![\s\S]))/m) || ''),
  };
}
    ...
    switch (kind) {
      ...
      case 'setup-cfg':
        ...
        for (const script of facts.scripts || []) {
          const module = facts.scriptModules?.[script];
          push(entrypoints, { name: script, files: [], kind: 'console-script', ...(module ? { module } : {}), source: file });
        }
        break;
      case 'pipfile':
