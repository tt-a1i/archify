...
// Nested e2e suites, pytest conftest/test_* files, and benchmark harnesses are
// not runtime code; their imports would otherwise become architecture edges.
const TEST_PATH = /(^|\/)(?:test|tests|__tests__|spec|specs|e2e|e2e[-_]tests?|integration[-_]tests?|bench|benches|benchmarks?)(\/|$)|(?:\.test|\.spec|_test|_tests)\.[^/]+$|(^|\/)(?:test_[^/]+|conftest\.py)$/i;
...
function modulePathOf(relativePath) {
  ...
  if (['src', 'app', 'lib', 'python'].includes(first) && segments.length >= 3) {
    return `${segments[0]}/${segments[1]}`;
  }
  return segments[0];
}
...
export function buildAggregatedConfiguration(root, records, limits) {
  ...
  for (;;) {
    ...
    batch = page.coverage.nextBatch;
  }
  const summary = summarizeBoundaries(configurations);
  return { configurations, boundaries: summary.boundaries, boundariesTruncated: summary.truncated, bytesRead };
}

export function buildRepositoryEvidence(root, options = {}) {
  const started = process.hrtime.bigint();
  const lap = (() => {
    let last = started;
    return () => {
      const now = process.hrtime.bigint();
      const elapsed = Number((now - last) / 1000000n);
      last = now;
      return elapsed;
    };
  })();
  ...
  if (options.detailDirectory) {
    ...
    const detail = {
      schemaVersion: 1,
      root: absoluteRoot,
      level1: { configurations: level1.configurations, boundaries: level1.boundaries, boundariesTruncated: level1.boundariesTruncated },
      level2,
      moduleRoles: Object.fromEntries([...classifyModules(level2, (level1.boundaries.entrypoints || [])
  ...
  // Import names with no matching file are only a finding when nothing
  // declares them: dependency lists from every parsed manifest say which
  // external names are expected.
  // A truncated dependency list cannot prove a name is undeclared, so the set
  // is withheld (null) whenever any dependency-bearing manifest was capped.
  let declaredDependencies = new Set();
  let sawDependencyManifest = false;
  for (const configuration of level1.configurations) {
    let hasDependencies = false;
    for (const key of ['dependencies', 'runtimeDependencies', 'requires']) {
      for (const name of configuration.facts?.[key] || []) {
        if (typeof name !== 'string') continue;
        hasDependencies = true;
        declaredDependencies?.add(name.split('.')[0].toLowerCase().replace(/-/g, '_'));
      }
    }
    if (hasDependencies && (configuration.factsTruncated || configuration.truncated)) declaredDependencies = null;
    if (hasDependencies) sawDependencyManifest = true;
  }
  // With no dependency manifest at all, every third-party import is
  // trivially "undeclared"; that is one repository fact, not a per-name lead.
  if (!sawDependencyManifest) declaredDependencies = null;
  const { fileEdges, ...level2Summary } = level2;
  const pack = buildEvidencePack({
    repositoryState: snapshot.repositoryState,
    summary,
    records,
    boundaries: level1.boundaries,
...
export function formatEvidencePack(result) {
  const pack = result.pack;
  const lines = [
    `Evidence pack: ${result.repository.root}`,
    `Coverage: ${pack.coverage.scannedFiles}/${pack.coverage.candidateFiles} scannable source files${pack.coverage.truncated ? ' (truncated)' : ''}; unscanned by language: ${JSON.stringify(pack.coverage.unscanned.byLanguage)}`,
    `Unresolved imports: ${pack.coverage.unresolved.stdlib} stdlib, ${pack.coverage.unresolved.external} external, ${pack.coverage.unresolved.dynamic} dynamic, ${pack.coverage.unresolved.unknown} unknown`,
    `Stage timings (ms): ${Object.entries(result.summary.timingsMs).map(([stage, ms]) => `${stage} ${ms}`).join(', ')}`,
    ...
    `Budget: ${pack.budget.bytes}/${pack.budget.maximumBytes} bytes${pack.budget.trimmed.length ? `; trimmed: ${pack.budget.trimmed.join(', ')}` : ''}`,
  ];
  return lines.join('\n');
}
