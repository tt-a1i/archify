...
test(
  ...
  async (t) => {
    ...
    for (const [width, height] of [
      ...
      assert.equal(
        await evaluate(`${frame}.getBoundingClientRect().bottom <= document.querySelector('nav').getBoundingClientRect().top`),
        true,
        "atlas navigation must not cover child reading content",
      );
      assert.equal(
        await evaluate(
      ...
      assert.equal(
        ...
        true,
        JSON.stringify(await evaluate(`(()=>{const w=${frame}.contentWindow,d=w.document.documentElement;return {width:innerWidth,height:innerHeight,child:[w.innerWidth,w.innerHeight,d.scrollWidth,d.scrollHeight],overflow:d.dataset.readerOverflow}})()`)),
      );
    }
...
test(
  ...
  async (t) => {
    ...
    const unreadable = original.replace(
      /(<text[^>]*class="t-backend"[^>]*font-size=")11("[^>]*>)/g,
      "$13$2",
    );
    assert.notEqual(unreadable, original, "the negative fixture must shrink message text");
    fs.writeFileSync(child, unreadable);
