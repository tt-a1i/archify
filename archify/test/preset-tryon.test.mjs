...
test('all five renderers expose one reader-controlled visual style picker', () => {
  for (const mode of Object.keys(CASES)) {
    ...
    for (const preset of ['classic', 'signal-flow', 'blueprint', 'editorial', 'blossom']) {
...
test('style selection synchronizes page, picker, and canonical SVG without touching geometry', () => {
  ...
  assert.match(runtime, /\['classic', 'signal-flow', 'blueprint', 'editorial', 'blossom'\]/);
...
test('same topology keeps identical canonical SVG geometry across all five styles', () => {
  const normalize = (svg) => svg.replace(/ data-preset="(?:classic|signal-flow|blueprint|editorial|blossom)"/, '');
  const variants = ['classic', 'signal-flow', 'blueprint', 'editorial', 'blossom'].map((preset) => normalize(svgBlock(render('architecture', preset))));
