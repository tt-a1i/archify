...
test('The evidence pack outlines likely reads with their length and top-level definitions', (t) => {
  const root = workspace(t);
  ...
  write(root, 'web/client.js', [
    "export async function load() { return fetch('/api/state'); }",
    'export class View {}',
  ...
  const app = pack.outlines.files['service/app.py'];
  assert.equal(app.lines, 8);
  assert.deepEqual(app.defs, ['2 Runner', '5 main', '7 __main__'], 'methods stay out of the top-level outline');
  assert.deepEqual(pack.outlines.files['web/client.js'].defs, ['1 load', '2 View', '3 render']);
  assert.match(pack.outlines.note, /cite only lines within lines/);
});
