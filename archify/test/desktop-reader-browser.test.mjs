    ...
    for (const scenario of [
      ...
      for (let attempt = 1; attempt <= scenario.attempts; attempt += 1) {
        const result = await runVisualCheck({
          ...
          chromePath,
          ...(scenario.browserFactory ? { browserFactory: scenario.browserFactory } : {}),
        });
        const context = `${scenario.label}, attempt ${attempt}: ${JSON.stringify(result.receipt, null, 2)}`;
        assert.equal(result.exitCode, 0, context);
        assert.equal(result.receipt.readability.status, 'pass', context);
