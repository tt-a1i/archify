...
test('a tall stacked workflow stays in the first desktop screen without shrinking below its SVG width', {
  skip: chromePath ? false : 'Set ARCHIFY_CHROME to run the real browser regression.',
}, async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archify-stacked-reader-'));
  const input = path.join(tmp, 'stacked.workflow.json');
  const artifact = path.join(tmp, 'stacked.html');
  try {
    fs.writeFileSync(input, JSON.stringify({
      schema_version: 2,
      diagram_type: 'workflow',
      meta: { title: 'Stacked stages', legend: { mode: 'hidden' } },
      lanes: [{ id: 'cage', label: 'One cage', height: 104 }],
      groups: [{
        id: 'group', label: 'Cage', lane: 'cage', fromCol: 1, toCol: 3, variant: 'security',
      }],
      nodes: [0, 90, 180].map((yOffset, index) => ({
        id: `stage-${index}`,
        lane: 'cage',
        col: 2,
        type: 'security',
    ...
    assert.ok(desktop, JSON.stringify(result.receipt, null, 2));
    assert.ok(desktop.diagramWidth >= 768, JSON.stringify(desktop));
    assert.ok(desktop.scrollHeight <= desktop.innerHeight, JSON.stringify(desktop));
    assert.equal(desktop.readabilityOk, true, JSON.stringify(desktop));
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
  }
});
