...
test('Level 1 also reads Python requirement files, Procfile, workspaces, Helm, GitLab and Azure CI, and sniffs Kubernetes YAML by content', (t) => {
  ...
  assert.deepEqual(byKind.get('setup-cfg').facts, { name: 'toolkit', dependencies: ['click', 'rich'], scripts: ['toolkit'], scriptModules: { toolkit: 'toolkit.cli' } });
...
test('Level 1 reads every pyproject dependency past extras brackets, markers, and comments', (t) => {
  const root = workspace(t);
  write(root, 'pyproject.toml', [
    ...
    'dependencies = [',
    '  "aiohttp",',
    '  "av==16.1.0 ; sys_platform == \'linux\' and (platform_machine == \'aarch64\')",',
    '  # a comment with [brackets]',
    '  "flashinfer_python[cu13]==0.6.18", # trailing ] comment',
    '  "transformers==5.12.1",',
    ']',
    '[project.optional-dependencies]',
    'test = ["pytest"]',
  ].join('\n'));
  const pyproject = buildRepositoryIndex(root).level1.configurations.find((entry) => entry.kind === 'pyproject');
  assert.deepEqual(pyproject.facts.dependencies, ['aiohttp', 'av', 'flashinfer_python', 'transformers']);
});
