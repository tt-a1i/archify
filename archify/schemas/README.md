...
detail. Once present, it requires a non-blank `initiator`, non-empty
`approvers` and `deliverables` arrays of non-blank facts, and a non-empty
`reworkPath` array of edge IDs. The workflow compiler rejects human-readable
facts repeated after Unicode and display-whitespace normalization, and requires
the edge IDs to be unique references that form a contiguous directed path
starting at that node. The field adds Semantic Passport detail and does not
alter layout geometry; visual exports remain overview-only.
