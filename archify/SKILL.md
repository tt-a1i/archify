...
3. Artifact first: the next tool action must write the candidate. Write the candidate before inspecting renderer internals. Do not plan exact coordinates in prose. Start with one clear main path, short side branches, sparse labels, and at most 12 primary nodes. Set `meta.quality_profile` to `"showcase"` unless the user explicitly requests a dense `standard` map. Start with automatic routes and labels. Do not add `via`, `channelX`, `channelY`, or `labelAt` before a diagnostic calls for one; apply at most one diagnosed geometry control per repair. **Plan-first mode for manual routing:** when validation reports geometry errors or visual inspection reveals edge overlaps, switch workflow before attempting fixes: (a) run `validate <type> <candidate.json> --layout-json` to obtain exact node rects and resolved edge polylines; (b) **prefer automatic over manual** — when edges overlap on the same node side, first check whether port spread is disabled for any of them by `route` presets, `via`, `channelX/Y`, or `labelAt`; if so, evaluate whether removing the disabling control lets spread resolve the overlap automatically — only proceed to manual `via` if spread cannot help; (c) read the route-preset behaviors and port-spread rules in `references/authoring-contract.md` (§ Executable geometry rules, § Spacing and labels) to understand what each route mode produces — do not trial-and-error route modes; (d) for each affected edge, calculate whether the planned route has sufficient channel width for labels (`mask width + 8px`) and clearance from adjacent segments (`≥4px`); (e) implement all spatially coupled geometry controls together as a batch, then validate once.
...
Do not read `renderers/shared/geometry.mjs`, renderer source, validator source, tests, or benchmarks before the first candidate. Inspect implementation only for an unsupported internal diagnostic or after two focused repairs fail. **Exception — plan-first mode:** when validation reports geometry errors requiring manual routing (non-auto `route`, `via`, or `fromSide`/`toSide` changes), read the route-preset and port-spread sections of `references/authoring-contract.md` and run `--layout-json` BEFORE attempting fixes. Understanding route-mode behavior first prevents blind trial-and-error.
