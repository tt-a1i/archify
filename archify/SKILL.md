...
4. Validate after every candidate edit and immediately before handoff. Before the final validation, run the **Authored-language gate** described under Authoring invariants when the authored language is not English, the artifact is bilingual, or the authored language differs from the Viewer locale. Account for every reader-facing authored string, preserve only exact technical identity, and finish with `unresolved_foreign_prose: 0`. When delegation is available, use an independent read-only language review; otherwise perform the same exhaustive inventory yourself. Any language repair changes the candidate and requires another validation.
...
- Choose one primary authored language from an explicit user choice, otherwise the request or conversation's dominant language. Separately choose the Viewer locale: use `meta.locale: "en"` or `"zh-CN"` for English or Simplified Chinese; a bilingual diagram still chooses one primary Viewer language. For every other authored language, omit `meta.locale`, keep every reader-facing authored string in the requested language except exact technical identity, and explicitly tell the user that fixed Viewer UI and `<html lang>` fall back to English and the artifact is not fully localized. Never treat that Viewer fallback as permission to fall back authored copy. See `references/authoring-contract.md` for the gate, field inventory, and technical-identity boundary.
...
`visual-check` measures containment, projected readability, and Viewer/Legend separation at 1440×900, 1600×1000, 1920×1080, and 2048×1320; captures light/dark screenshots at the smallest and largest sizes; and writes a relative-path contact sheet plus JSON sidecars beside the artifact. Its automated receipt always reports `visualReview: "pending"`: screenshots are evidence for inspection, never an automatic polish claim. Exit 0 means every automated gate and capture passed, 1 means an automated gate or capture failed, and 2 means Chrome/Chromium was unavailable and the receipt is `skipped`. The command never changes the delivered HTML.
...
Return the checked HTML path, diagram type, validation summary, specification/artifact receipt, truthful visual-review status, and the Authored-language review receipt when its gate applies. Do not claim success for a non-zero command or claim visual inspection you did not perform.
