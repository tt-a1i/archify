...
- Choose one primary authored language from an explicit user choice; otherwise follow the request or conversation's dominant language; **absent any other signal, default to Korean (한국어)** — this installation's standing preference. Content stays in the chosen language throughout: titles, node/participant labels, sublabels, edge labels, view notes, and card text. `meta.locale` controls renderer-owned Viewer UI (buttons, legend, view-picker, `<html lang>`): set it to `"ko"` when authoring in Korean — the full catalog (519 strings) is translated, so Viewer chrome renders in Korean too, not just diagram content. `"en"` and `"zh-CN"` remain the other two supported values. For any other, unsupported language, omit `meta.locale` and disclose once, briefly, that Viewer UI chrome falls back to English. The renderer never translates authored content. See `references/authoring-contract.md` for details.
...
`renderers/shared/i18n.mjs` carries a third locale, `'ko'`, alongside `'en'` and `'zh-CN'` — every one of its 519 Viewer-chrome strings (toolbar, legend, guided views, route/lens/radar, exports, passport, finder…) is translated, not just a subset. Set `meta.locale: "ko"` and the rendered Viewer UI itself — not only authored content — comes up in Korean.

`assets/template.html` bundles Pretendard Variable (SIL OFL, `assets/Pretendard-OFL.txt`) alongside JetBrains Mono, inserted right after it in every rendered font stack. Latin/digits still render in JetBrains Mono; any Hangul — in authored content or in the now-Korean Viewer chrome — falls through automatically to Pretendard. No per-diagram configuration needed for either the font or the translation catalog; both are baked into the template. Nothing to do here beyond writing Korean text as normal and setting `meta.locale: "ko"`.
