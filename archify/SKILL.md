...
- Omit `meta.visual_preset` by default so every diagram opens in `classic`, regardless of whether its resolved color mode is light or dark. Color mode and visual preset are independent: switching Light / Dark must preserve the current preset. Set `signal-flow`, `blueprint`, `editorial`, or `salesforce` only when the user explicitly requests that visual style. The `salesforce` preset is a Salesforce-inspired Soft Cloud treatment, not an official Salesforce artifact.
