...
- For a business approval gate only, use the node's optional `approval` object when initiator, approvers, required deliverables, and the rework path are all decision-critical. Reference the existing directed rework edges by ID in `reworkPath`; never duplicate those facts in cards. Omit this field for ordinary workflows.
