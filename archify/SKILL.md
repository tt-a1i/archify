...
- Choose one primary authored language from an explicit user choice; otherwise follow the request or conversation's dominant language. `meta.locale` controls only renderer-owned Viewer UI: `"en"`, `"zh-CN"`, and `"es"` are built in; any other language tag needs `meta.translations` supplying the renderer-owned message catalog as data (see "Localizing the Viewer UI with meta.translations" below), or the fixed Viewer UI and `<html lang>` fall back to English — disclose that fallback explicitly when it applies. The renderer never translates authored content. See `references/authoring-contract.md` for details.
...
## Localizing the Viewer UI with meta.translations

This section is about renderer-owned Viewer chrome only. Write titles, node labels, cards, and every other authored string directly in whatever language the user explicitly requested or the conversation established — Archify never machine-translates authored content or calls a translation service, before or after delivery.

**Any well-formed locale tag works, for any language — not just the ones listed below.** `en`, `zh-CN`, and `es` ship as built-in Viewer-chrome catalogs (`renderers/shared/i18n.mjs`), so those three need nothing further. Every other locale — including ones with no example on disk — is supplied as translation *data* you (the authoring agent) write into the document, not a renderer code change and not a fixed list anywhere in this repo. To localize a diagram in a language with no existing example, set `meta.locale` to its tag and write `meta.translations` yourself: an object mapping canonical message keys (see `catalogKeys()`, or any key in `examples/locales/ko.json`) to translated strings whose `{placeholder}` tokens match the English source exactly. `validate`/`render`/`deliver` report coverage to stderr (`archify: meta.translations for locale "…" covers N/519 renderer-owned messages…`); a key that is missing, unknown, or has mismatched placeholders falls back to its English string rather than breaking the render. `meta.locale` without a built-in catalog or `meta.translations` falls back entirely to English Viewer UI and `<html lang="en">`, with the same disclosure on stderr — never a half-translated page. The renderer itself never translates anything or calls a translation service; every string in `meta.translations` must already be present in the document when you call `validate`/`render`/`deliver`.

`examples/locales/` holds format references only, not catalogs to copy: `examples/locales/ko.json`, `fr.json`, `pt.json`, `ja.json`, `de.json`, `it.json`, and `ru.json` each show a complete (519/519 keys) catalog, and `examples/locales/fr.partial.json` is a small, deliberately partial French catalog demonstrating the missing-key/coverage-report path specifically. Use them to see the key set, placeholder shape, and coverage behavior — never paste one into `meta.translations`. Translate the Viewer strings yourself for each diagram, in the target language and in keeping with that diagram's own terminology, even when an example file exists for that language. None of these are a closed enum; a language with no file here works exactly the same way.

`assets/template.html` also bundles D2Coding (SIL OFL, `assets/D2Coding-OFL.txt`) after JetBrains Mono in the monospace stack, and one weight of Pretendard (SIL OFL, `assets/Pretendard-OFL.txt`) in the "editorial" preset's serif heading stack, so Hangul in authored content or a Korean-translated chrome renders without missing glyphs. This is independent of `meta.translations`: it helps any locale or authored text that uses Hangul, and is not required for the translation mechanism itself to work.
