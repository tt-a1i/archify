...
- `explanation` is optional per-node plain text for the Semantic Passport's Explain action: one to three short paragraphs separated by blank lines, no markdown, no links, at most 1200 characters. Author it only where a reader needs context the label and sublabel cannot carry; it never enters the SVG, exports, or layout.
