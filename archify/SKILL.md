---
name: archify
description: Create audience-first, validated architecture, workflow, sequence, data-flow, and lifecycle/state explanations as polished standalone HTML with inline SVG, dark/light themes, optional trace motion, and PNG/JPEG/WebP/SVG/WebM export. Accept plain-language requirements or pasted Mermaid flowchart, sequenceDiagram, and stateDiagram input; inspect repository evidence when the diagram must reflect real code. Use when the user asks to explain or visualize system architecture, infrastructure, cloud/security/network topology, technical workflows, API call sequences, request lifecycles, data pipelines, ETL/ELT, data lineage, state machines, or to convert/beautify Mermaid.
license: MIT
metadata:
  version: "2.16"
  author: tt-a1i
...
Create a self-contained, interactive HTML explanation from a small typed JSON specification. The diagram must stand on its own for its intended reader; visual polish cannot compensate for a story that still needs the author beside it. Static output is the default; enable motion only when the user asks for a demo or presentation.

## Fast authoring path

Use this bounded path for ordinary generation. Do not read the optional Viewer Runtime reference unless the user asks about those features.

1. Freeze the reader contract before choosing geometry: name the primary audience, the one conclusion or decision they should carry away, and the question the main path must answer. Infer these from the request when clear; ask only when different answers would materially change the artifact. For a mixed audience, make the least technical intended reader the default and preserve expert detail as a second layer.
2. Choose `architecture`, `workflow`, `sequence`, `dataflow`, or `lifecycle` from the question.
...
4. Artifact first: the next tool action must write the candidate. Write the candidate before inspecting renderer internals. Do not plan exact coordinates in prose. Start with one clear main path that answers the reader's question, short side branches, sparse labels, and at most 12 primary nodes. Set `meta.quality_profile` to `"showcase"` unless the user explicitly requests a dense `standard` map. Start with automatic routes and labels. Do not add `via`, `channelX`, `channelY`, or `labelAt` before a diagnostic calls for one; apply at most one diagnosed geometry control per repair.
5. Run the reverse-reader check on the candidate before final validation. Pretend you are the named reader and have no author present. For every transition, answer: “what is this?”, “why do we go here?”, and “what results next?”. Every fan-out must state why the flow separates; every convergence must make the inputs and combined result evident. Rewrite or remove anything that cannot answer those questions without oral explanation.
6. Validate after every candidate edit and immediately before handoff:
...
7. For a delivered HTML, `deliver` is the final acceptance command:
...
- Author for the reader's conclusion, not for the inventory of technologies. Keep exact product, protocol, and system names correct, but make plain-language purpose or consequence the primary label when the intended reader is non-technical. Preserve the exact technical name in reader-invoked detail when the schema supports it; keep it visible only when that identity is needed to understand or verify the flow.
- A branch is a claim, not decoration. Label outgoing relationships with the causal condition, responsibility, input, or reason that distinguishes them. Two arrows from one node without a reader-visible reason are incomplete even when the topology is technically correct.
- Show parallel sources as parallel sources when they contribute different things. Name what each supplies, keep their paths separate, and show the convergence point where they are combined. Do not flatten concurrent inputs into a false sequence or imply that one source produces the other.
- Use progressive disclosure. The default READ view should communicate the business picture, main path, branch reasons, convergence, and outcome. For a non-technical default, keep primary labels and sublabels in plain language; put definitions, implementation detail, and non-essential exact technical identifiers in optional `meta.views` chapters or another schema-supported reader-invoked detail. Keep an exact technical name on the main canvas only when that identity is needed to understand or verify the flow. Do not hide facts needed to understand why the path exists.
- Keep the main story monotonic left-to-right or top-to-bottom. Use branches for real alternatives or parallel inputs and return them to an explicit outcome where the process actually converges. Technical detail may expand the story but must not interrupt its reading order.
...
Return the checked HTML path, diagram type, intended audience, reader takeaway, `reverse_reader_review: passed|failed`, validation summary, specification/artifact receipt, and truthful visual-review status. Mark the reverse-reader review as passed only after checking every transition, branch reason, convergence, and outcome without relying on oral explanation; name the first unresolved transition when it fails. Do not claim success for a non-zero command or claim visual inspection you did not perform.
