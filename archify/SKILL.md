...
- **Clean flow before delivery**: a relationship may touch its own source/target boxes but must not pass through any unrelated semantic node. Fix `clean-flow/edge-through-node` with `fromSide`/`toSide`, a route/channel, explicit `via`, or node placement; do not hide the defect with paint order.
- **Choose a composition profile deliberately**: `standard` keeps dense engineering topologies renderable and records route-rhythm/proper-X findings as warnings; `showcase` fails unrelated proper X crossings, any segment below 8px, and interior turn segments below 16px. Endpoint stubs from 8–15px remain valid because fixed lane gaps can require them. Use `meta.quality_profile` or CLI `--quality`, then repair with route/channel/via/side or placement controls.
