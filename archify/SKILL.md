...
Entity-relationship note: a relationship reads `from` -> `to`, and both ends must declare their own maximum — `fromCardinality`/`toCardinality` take `one` or `many` and are required, because a maximum you never verified is a fact the diagram would be inventing. An `fk` attribute must likewise name its target in `references` as `"entity.attribute"`. `fromOptional`/`toOptional` only lowers that end's minimum from one to zero, so an optional `one` end reads zero-or-one and an optional `many` end reads zero-or-many. Set `identifying: false` for a dashed non-identifying line. Entity boxes are opaque: a route that would pass through an unrelated entity is routed around it, and a route that cannot clear it is an error rather than a drawn crossing. For a database-schema ERD, the complete field list is the primary content and is visible at the default `read` level; cards are supplementary notes only. The schema has no separate attribute-comment property, so preserve comments as `type: "SQL_TYPE｜中文备注"` (append `｜可空` when applicable) rather than dropping them. First group tables by verified business/module meaning, keep each `tag` block contiguous and non-interleaved, put parent/child tables near the block boundary that carries the relationship, and leave a routing row or column between large blocks when possible. Do not use an empty `sublabel` to suppress a useful domain tag.
...
| `erd` | Tables/entities, attributes, primary/foreign/unique keys, cardinality between them |
