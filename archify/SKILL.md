...
- Author for the reader's conclusion, not for the inventory of technologies. Keep exact product, protocol, and system names, but make plain-language purpose or consequence the primary label when the intended reader is non-technical. Put the exact technical name in a concise secondary label, tag, or reader-invoked detail when the schema supports it.
...
- Use progressive disclosure. The default READ view should communicate the business picture, main path, branch reasons, convergence, and outcome. Put definitions, implementation detail, and exact technical identifiers in secondary labels or optional `meta.views` chapters so readers can request depth without making the first screen a glossary. Do not hide facts needed to understand why the path exists.
