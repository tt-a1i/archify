...
- Architecture boundaries reserve a readable title band automatically. Nested boundaries inferred from strict `wraps` containment expand their nearest parent; partially overlapping frames separate coincident edges, and automatic routes push nearby parallel borders outward. Do not inflate `pad` merely to separate boundary titles, borders, or route corridors.
...
- Showcase rejects a near-axis dogleg when facing automatic ports end less than 24px apart but the route still bends. Align intended vertical or horizontal pairs by their component centers, reserve one facing port for the direct pair, and move side branches to another truthful side; do not accept a legal dogleg when the same topology has a clear straight corridor.
