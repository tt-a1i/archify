...
- Brand identity is optional and explicit. Put a canonical built-in ID in `brand` when the node names that real product. If no preset matches and the user supplied the official HTTP(S) URL, first run `node bin/archify.mjs brands capture "<url>" --json`, then author the returned digest-pinned `brand` object. Render and validate never perform an unpinned capture. Otherwise omit `brand`. If a known brand is unavailable on rights `HOLD`, keep its product name in `label` and consider the receipt's neutral `capability` suggestion only after confirming it describes the node; list supported IDs with `node bin/archify.mjs capabilities --json`. `brand` and `capability` are mutually exclusive, and Archify never infers one from the other, a label, alias, or URL. Never let either badge replace the semantic `type`, label, or relationship facts.
