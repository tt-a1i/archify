...
`en`, `zh-CN`, and `es` ship as built-in Viewer-chrome catalogs (`renderers/shared/i18n.mjs`); every other locale is supplied as translation *data*, not a renderer code change. Set `meta.locale` to any well-formed language tag and, unless it is `en`, `zh-CN`, or `es`, also set `meta.translations` to an object mapping canonical message keys (see `catalogKeys()`, or any key in `examples/locales/ko.json`) to translated strings whose `{placeholder}` tokens match the English source exactly. `validate`/`render`/`deliver` report coverage to stderr (`archify: meta.translations for locale "…" covers N/519 renderer-owned messages…`); a key that is missing, unknown, or has mismatched placeholders falls back to its English string rather than breaking the render. `meta.locale` without a built-in catalog or `meta.translations` falls back entirely to English Viewer UI and `<html lang="en">`, with the same disclosure on stderr — never a half-translated page.

`examples/locales/ko.json` is a complete, checked-in Korean catalog (519/519 keys) — the worked example for this contract. `examples/locales/fr.partial.json` is a small, deliberately partial French catalog proving the mechanism needs no renderer or schema change to support a new, previously-unsupported language: only translation data.
