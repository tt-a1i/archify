...
`examples/locales/` carries reusable, checked-in catalogs so a diagram doesn't need retranslating from scratch every time: `examples/locales/ko.json`, `fr.json`, `pt.json`, `ja.json`, `de.json`, `it.json`, and `ru.json` are each complete (519/519 keys). `examples/locales/fr.partial.json` is a small, deliberately partial French catalog demonstrating the missing-key/coverage-report path specifically. None of these are a closed enum — they're starting points to copy or extend, and a language with no file here works exactly the same way once you write its `meta.translations`.
