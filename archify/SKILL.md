...
- Architecture may give a top-level component one optional `subarchitecture` when the user needs an exact, authored drill-down. This field is strictly additive: preserve every existing parent component, boundary, connection, card, and authored coordinate, and never move a parent-level node into the child graph merely to make the overview smaller. Keep the child to one level and 1–12 local components; local IDs are unique within that parent, and every local boundary member and connection endpoint stays in the same scope. Never add nested subarchitectures or parent/child cross-scope edges. The ordinary parent interaction remains unchanged; the focused component's Semantic Passport provides the explicit internals entry. Treat the drill-down as Viewer-only. Print and embed remain parent-only; exports default to the main architecture, while an open child may be exported explicitly as a child-only PNG, JPEG, WebP, SVG, clipboard image, or Share Card. Never combine parent and child output, and keep Route, Reachability, and WebM parent-only. Read `examples/transformer-layer.architecture.json` for the field shape.
