...
Editable Source Capsule is an explicit handoff choice, never a default. When the user wants a recipient to continue editing from the standalone HTML, offer the choice to add `--with-source` to `render` or `deliver`; if the sharing or privacy boundary is unclear, ask before enabling it. Otherwise keep it off. The option embeds only the already validated JSON value plus the input basename—never an absolute path—and reveals **Export → Source JSON** only in that artifact. The JSON is inert, script-closing text must be escaped before insertion, malformed capsules fail closed, and the delivery receipt must report `sourceCapsule.included: true`. Source data is confined to that opted-in HTML and must never enter SVG, PNG, JPEG, WebP, WebM, Share Card, Route Share Card, URLs, storage, logs, or default Proof Lab artifacts.
