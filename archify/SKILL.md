...
- For repository-backed Architecture, author `semanticChecks` from verified evidence before layout. Include every material entrypoint or controller family, lifecycle scheduler/background worker, and direct external-system management path as `requiredComponents`, `requiredEdges`, or `requiredPaths`; set `requireExternalLabels` when external operations must be named. Resolve every coverage warning by adding the fact or recording a specific, reasoned `omissions` entry. Never use an omission merely to silence a warning.
