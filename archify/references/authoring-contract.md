...
English, `"zh-CN"` for Simplified Chinese, or `"zh-TW"` for Traditional
Chinese. The renderer consumes the authored
...
For a requested language outside `en`, `zh-CN`, and `zh-TW`, do not write an
unsupported
...
`zh-CN` for another language or Chinese locale, and never substitute `zh-CN`
for `zh-TW` or the reverse: they differ in script and in regional terminology.
