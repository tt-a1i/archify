...
English, `"zh-CN"` for Simplified Chinese, or `"pt"` for Portuguese, whose
Viewer copy is written in European Portuguese and published as
`<html lang="pt-PT">`. The renderer consumes the authored locale without
inferring language from diagram strings. Documents that omit it remain valid
...
For a requested language outside `en`, `zh-CN`, and `pt`, do not write an unsupported
...
`zh-CN` for another language or Chinese locale, and do not substitute `pt` for
