...
- **Anti-parallel connection caveat**: Two connections that share the same endpoints but run in opposite directions (e.g., A→B and B→A) can resolve to the same emitted geometry. Showcase quality measures the emitted routes and reports a warning as `composition/coincident-routes`; a shared line is allowed when the labels clearly attribute direction. The check keys on measured coincidence, so explicit `via`, `channelX`/`channelY`, or `fromSide`/`toSide` geometry that keeps the pair apart passes, whether or not the edges carry `labelAt`. Removing `labelAt` is not a reliable repair: with one label left, the surviving labeled edge can remain coincident; with both removed, labels may fail clearance. When the warning identifies real ambiguity, separate the routes explicitly — for example give one edge `fromSide: "top"`, `toSide: "top"`, `via: [[180,100],[530,100]]`, and `labelAt: [355,80]` so it arcs around the other.
