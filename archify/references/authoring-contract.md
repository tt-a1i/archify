...
- **Anti-parallel connection caveat**: When two connections share the same endpoints but run in opposite directions (e.g., A→B and B→A), and both carry `labelAt`, their routes will coincide, rendering as a single double-headed line that readers cannot distinguish. Showcase quality rejects this as `composition/coincident-routes`. To maintain visually separate routes for anti-parallel pairs, remove `labelAt` from at least one connection to enable automatic port spreading, or use explicit `via` points or `channelX`/`channelY` offsets.
