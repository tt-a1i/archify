...
- Relationship Exploration keeps direct outgoing/incoming endpoints and authored upstream/downstream results outside the visible Passport. The Passport is bounded to the container's visible height: its title and actions stay fixed, one scroll body owns overflow, a non-blocking fade signals more content, and outgoing/incoming/self-loop groups use a single-open accessible accordion. Authored reachability and pinned relationships use a compact Passport summary with an explicit Details toggle; the semantic camera fits every active fact into the remaining safe viewport, even when that requires going below the preferred reading scale, and restores a mostly off-screen diagram before reframing. Extremely short viewports collapse to a focus-safe summary instead of clipping the fixed header. Manual pan or zoom still takes control.
