...
const arch = {
  ...input,
  meta: { ...inheritedMeta, ...(input?.meta || {}) },
};
const identityOptions = { identityPrefix };
...
function renderConnectionPath(conn, index) {
  ...
  return `        <path ${focusEdgeAttrs(conn.from, conn.to, conn.label, index, conn.id)} data-composition-points="${routePointsValue(routed.points)}" d="${routed.d}" class="${cls}"${animateAttr(arch.meta, 'edge', index)} stroke-width="${strokeWidth}" marker-end="url(#${scopedSvgId(marker, identityOptions)})"/>`;
}
...
function renderComponent(c) {
  ...
  return `        <g ${focusNodeAttrs(c.id, c.label, passport, arch.meta.locale, identityOptions)}>
...
function renderSvg() {
  return `      <svg viewBox="0 0 ${viewBox[0]} ${viewBox[1]}" ${svgRootAttrs(arch.meta, identityOptions)}>
${svgAccessibleText(arch.meta, 'architecture', identityOptions)}
${renderDefinitions(identityOptions)}

        <!-- Background Grid -->
        <rect width="100%" height="100%" fill="url(#${scopedSvgId('grid', identityOptions)})" />
...
const layoutReport = buildLayoutReport();
const svg = renderSvg();
...
const semantic = {
  ...
  parentId,
  sourceEvidence,
  nodes: [...components.values()].map((component) => ({
    id: component.id,
    label: component.label,
    kind: component.type,
    sublabel: component.sublabel ?? null,
    tag: component.tag ?? null,
    context: componentContext(component),
    ...brandMetadataFor(component),
  })),
  edges: asArray(arch.connections).map((connection, index) => ({
    id: connection.id ?? null,
    key: index,
    from: connection.from,
    to: connection.to,
    label: connection.label ?? null,
    variant: connection.variant || 'default',
  })),
};
