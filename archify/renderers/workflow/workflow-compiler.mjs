...
function compileWorkflowInternal({
  ...
  // Colours for lane-ownership accent bars — one per lane row, cycling through
  // the same semantic stroke CSS vars used by edges and node fills.
  ...
  // Map from phase variant to CSS custom-property stroke colour.
  ...
  /**
   * Render a 3px left-side accent bar per lane row.
   * Derived from lanes[]/nodes[].lane — fires for every workflow, even those
   * with no `phases` array (the exact case from issue #81).
   */
  function renderLaneOwnershipBars() {
    return asArray(workflow.lanes).map((lane, index) => {
      ...
      return `        <rect data-lane-band="${esc(lane.id)}" x="${layout.laneX}" y="${y}" width="3" height="${height}" fill="${color}" fill-opacity="0.9"/>`;
    }).join('\n');
  }

  /**
   * When `phases` is defined, additionally render a low-opacity column band per
   * phase spanning every lane row — one rect per lane to avoid tinting routing gaps.
   */
  function renderPhaseBand(phase) {
    ...
    return asArray(workflow.lanes).map((_lane, index) => {
      const y = laneTop(workflow.lanes[index].id);
      const height = laneHeight(index);
      return `        <rect data-phase-band="${esc(phase.id)}" x="${span.x}" y="${y}" width="${span.width}" height="${height}" rx="6" fill="${color}" fill-opacity="0.055" stroke="${color}" stroke-opacity="0.18" stroke-width="1" stroke-dasharray="4 3"/>`;
    }).join('\n');
  }
