// Orthogonal connection router shared by the typed renderers.
//
// Extracted from the architecture renderer so a second diagram type reuses the
// same candidate families, endpoint-side contract, and obstacle clearance
// instead of copying them. Every function here is pure and receives the
// component map it must avoid; the architecture renderer's own coordinates and
...
function alignFacingPorts(conn, from, to, start, end, fromSide, toSide, ports, components) {
  ...
  for (const candidate of candidates) {
    ...
    if (portHasCornerClearance(from, fromSide, candidate.start)
        ...
        && routeClearsComponents(conn, points, components)) {
      return candidate;
    }
  }
  return { start, end };
...
function routeVia(conn, from, to, start, end, fromSide, toSide, { components, preferredCandidates, extraCandidates, strictClearance = false } = {}) {
  ...
  switch (conn.route || 'auto') {
    ...
    case 'auto':
    default: {
      // A type may own the channel choice for relationships that would
      // otherwise share one corridor. Preferred candidates are tried before the
      // shared families, so an explicit lane wins over the generic midpoint;
      // a candidate that violates the endpoint contract or an obstacle is
      // simply skipped and the historical order below still applies.
      if (typeof preferredCandidates === 'function') {
        for (const candidate of preferredCandidates({ conn, from, to, start, end, fromSide, toSide }) || []) {
          ...
          if (routeHonorsEndpointSides(points, fromSide, toSide)
            && routeClearsEndpointComponents(points, from, to)
            && routeClearsComponents(conn, points, components)) return candidate;
        }
      }

      // Direct line unless the anchors are clearly orthogonal-friendly.
      //
      // A type that can route around obstacles (strictClearance) only takes the
      // direct line when that line is genuinely clear, so an entity sitting
      // between two aligned anchors gets a detour instead of a
      // clean-flow/edge-through-node failure. The historical behaviour — draw
      // straight and let the Clean Flow gate name the obstacle — is the default.
      ...
      if ((deltaX < 4 || deltaY < 4) && routeHonorsEndpointSides([start, end], fromSide, toSide)) {
        if (!strictClearance
          || (routeClearsEndpointComponents([start, end], from, to)
            && routeClearsComponents(conn, [start, end], components))) {
          return [];
        }
      }
      ...
      if (fromVerticalSide && toVerticalSide && deltaX < minimumStub * 2) {
        ...
        for (const channelX of outsideChannels) {
          ...
          const points = [start, ...candidate, end];
          if (routeHonorsEndpointSides(points, fromSide, toSide) && routeClearsComponents(conn, points, components)) return candidate;
        }
      }
      ...
      for (const candidate of ordered) {
        const points = [start, ...candidate, end];
        if (routeClearsEndpointComponents(points, from, to) && routeClearsComponents(conn, points, components)) return candidate;
      }

      // A type may supply its own candidate family. It runs only after the
      // shared families fail, so an ordinary diagram keeps its exact historical
      // routing, while a denser one (the ERD renderer's per-lane corridors)
      // still gets a chance to route legally instead of failing the gate.
      if (typeof extraCandidates === 'function') {
...
// Port spreading, side selection, and route caching for one diagram.
//
// `sideFor(relation, endpoint)` lets a type bias which side an endpoint prefers
// (the ERD renderer keeps relationships on left/right); when it is omitted
// every decision falls back to the shared default, so existing callers keep
// their exact coordinates. `extraCandidates` runs only after the built-in
// automatic candidates are exhausted and before the best-effort fallback.
export function createOrthogonalRouter({
  ...
  preferredCandidates,
  extraCandidates,
  strictClearance = false,
  rounding = 8,
} = {}) {
  const ports = automaticPortSpread(
    relations,
    components,
    sideFor ? { sideFor } : undefined,
  );
  const cache = new Map();
  ...
  function pathFor(conn) {
    ...
    const from = components.get(conn.from);
    const to = components.get(conn.to);
    const automaticPorts = ports.get(conn);
    const { fromSide, toSide } = connectionSides(conn);
    const baseStart = automaticPorts?.from || anchor(from, fromSide);
    const baseEnd = automaticPorts?.to || anchor(to, toSide);
    ...
    const routed = { d: roundedPath(points, rounding), points, fromSide, toSide };
    cache.set(conn, routed);
    return routed;
  }

  return { ports, pathFor, connectionSides, connectionEndpointSide, cache };
}

// Low-level pieces exported so a type can compose its own candidate family
// without reimplementing the endpoint-side contract or the clearance tests.
export {
  routeClearsComponents,
  routeClearsEndpointComponents,
  routeVia,
  alignFacingPorts,
  portHasCornerClearance,
